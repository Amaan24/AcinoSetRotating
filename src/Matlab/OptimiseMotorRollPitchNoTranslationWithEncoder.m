...
project = '25Apr2023';
camera = 2; %Camera 1 or 2
%frame1 = 10490;

%frames = [12200, 12225, 12250, 12275]; %Camera 1
%frames = [10490, 12200, 12220, 12230, 12240, 12250 12270, 12290, 12300, 12310, ... 
%    12710, 12730, 12750, 12770, 12790, 12800]; %Camera 1
frames = [10490, 10500, 10510, 10520, 10530, 10540, 10550, 10560, 10570, 10580, ...
    10590, 10600, 10620, 10900, 10910, 10940, 10960, 10980]; %Camera 2
...
elseif (camera == 2)
    ...
    vidIn = VideoReader('C:\Users\user-pc\Desktop\AcinoSetRotating\data\25Apr2023\2.avi');
...
%Solve optimisation
RCWInitial = [0 0];
encoderNoiseInitial = 0 .* ones(1, length(frames));
XInitial = [RCWInitial, encoderNoiseInitial];

b_angles = 0*pi/10 .* ones([1,2]); %Radians
b_enc_angles = 0.0174533 .* ones([1,length(frames)]); 

ub = [b_angles b_enc_angles];
lb = -1 .* ub;
...
R_CM_homog = [R_CM t_CM; [0, 0, 0, 1]]; %Camera to motor
R_MC_homog = [R_MC t_MC; [0, 0, 0, 1]];

for i = 1:length(frames)
   pic = read(vidIn, frames(i));
    imshow(pic);
    title("Frame " + frames(i) + "(" + i + "/" + length(frames) + ")")
    axis on
    hold on;

    %Optimised
    R_MM_homog = [rot_y(angles(frames(i)) + EncoderNoise(i)).' [0; 0; 0]; [0, 0, 0, 1]];
...
function r = residual(X, Ri, ti, enc_angles, intrinsics, cornerPoints, worldPoints, frames)
    r_pixels = zeros(20, 2, length(frames));
    for i = 1:length(frames)
        angles = X(1:2);
        c_CM = [0;0;0];
        ...
        R_CM_homog = [R_CM t_CM; [0, 0, 0, 1]]; %Camera to motor
        R_MM_homog = [rot_y(enc_angles(frames(i)) + encoderNoise).' [0; 0; 0]; [0, 0, 0, 1]]; 
        R_MC_homog = [R_MC t_MC; [0, 0, 0, 1]];
        ...
        for j = 1:20 %Number of checkerboard corners
            for k = 1:2 %Dimensionality of measurements
                r_pixels(i,j,k) = differences(j,k);
            end
        end
    end
    r_pixels = reshape(r_pixels, 1, []);
    r_pixels = 1/(0.5^2) .* r_pixels;

    r_encoder = X(3:end);
    r_encoder = 1/(0.0174533^2) .* r_encoder;
    r = [r_pixels, r_encoder];
end
