...
%Solve optimisation
RCWInitial = [0 0];
CmInitial = 100 .* ones(1);
encoderNoiseInitial = 0 .* ones(1, length(frames));
XInitial = [RCWInitial, CmInitial, encoderNoiseInitial];

b_angles = pi/10 .* ones([1,2]); %Radians
b_translation = 100 .* ones(1); %Millimeters
b_enc_angles = 0.0174533 .* ones([1,length(frames)]); 

ub = [b_angles  b_translation b_enc_angles];
lb = -1 .* ub;
