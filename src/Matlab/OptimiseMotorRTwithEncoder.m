...
%Add backlash to encoder
%disp(angles(1))
%angles = -0.0174533 + angles;
%disp(angles(1))

%Solve optimisation
RCWInitial = [0.1 0.1 0.1];
CmInitial = 0 .* ones(1,3);
encoderNoiseInitial = 0 .* ones(1, length(frames));
XInitial = [RCWInitial, CmInitial, encoderNoiseInitial];

b_angles = pi/10 .* ones([1,3]); %Radians
b_translation = 100 .* ones([1,3]); %Millimeters
...
%Display results
%Encoder noise
EncoderNoise = X_solution(7:10);

%Camera at t0 to motor frame
R_CM = rot_y(X_solution(2)) * rot_x(X_solution(1)) * rot_z(X_solution(3));
c_CM = X_solution(4:6).';
t_CM = -R_CM * c_CM; 
...
function r = residual(X, Ri, ti, enc_angles, intrinsics, cornerPoints, worldPoints, frames)
    ...
    for i = 1:length(frames)
        ...
        %Camera at t0 to motor frame
        R_CM = rot_y(angles(2)) * rot_x(angles(1)) * rot_z(angles(3));
        t_CM = -R_CM * c_CM; 
