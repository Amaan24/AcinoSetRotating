...
def build_model(project_dir) -> ConcreteModel:
    ...
    p_chin         = p_head         + R0_I  @ sp.Matrix([0, 0, -0.19])
    p_neck         = p_head         + R0_I  @ sp.Matrix([0, 0, -0.30])
    p_shoulder1    = p_neck         + R2_I  @ sp.Matrix([0, 0.21, 0])
    p_shoulder2    = p_neck         + R2_I  @ sp.Matrix([0, -0.21, 0])
    p_elbow1       = p_shoulder1    + R3_I  @ sp.Matrix([0, 0, -0.3])
    p_elbow2       = p_shoulder2    + R4_I  @ sp.Matrix([0, 0, -0.3])
    p_wrist1       = p_elbow1       + R5_I  @ sp.Matrix([0, 0, -0.26])
    p_wrist2       = p_elbow2       + R6_I  @ sp.Matrix([0, 0, -0.26])
    p_pelvis       = p_neck         + R2_I  @ sp.Matrix([0, 0, -0.6]) #check this
    p_hip1         = p_pelvis       + R7_I  @ sp.Matrix([0, 0.15, 0])
    p_hip2         = p_pelvis       + R7_I  @ sp.Matrix([0, -0.15, 0])
    ...
    x_est = np.array([forehead_points[0][0]  for i in range(len(frame_est))])
    y_est = np.array([forehead_points[0][1]  for i in range(len(frame_est))])
    z_est = np.array([forehead_points[0][2]  for i in range(len(frame_est))])
    ...
    def enc_measurement_constraints(m, n, p):
        if (p < 45):
            ...
            return  m.x[n, p] - m.meas_enc[n, P - p] == 0
