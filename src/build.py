...
def build_model(skel_dict, project_dir) -> ConcreteModel:
    ...
    scene_path = os.path.join(project_dir, "data/11Oct2022S", "extrinsic_calib", "2_cam_scene_sba.json")
    ...
    df_paths = sorted(glob.glob(os.path.join(project_dir, "data/11Oct2022S", '*.h5')))
...
def solve_optimisation(model, exe_path, project_dir, poses) -> None:
    ...
    result_dir = os.path.join(project_dir, "data/11Oct2022S", "results")
...
if __name__ == "__main__":
    ...
    print(skelly)
    data_path = os.path.join(args.top_dir, "data/11Oct2022S")
